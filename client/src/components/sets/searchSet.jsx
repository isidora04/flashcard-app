...
const SearchSet = ({ flashcardSet }) => {
    ...
    { !user && <p>Loading set...</p>}

    return (<>
        <div className={styles.cardContainer}>
            <div className={styles.cardContent}>
                <div className={styles.cardHeader}>
                    <h3 onClick={handleInfo} className={styles.cardTitle}>{flashcardSet.title}</h3>
                    <p className={styles.numCards}>{flashcardSet.num_cards} {flashcardSet.num_cards === 1 ? "card" : "cards"}</p>
                </div>
                <div className={styles.cardInfo}>
                    <div className={styles.cardUser}>
                        <button>{flashcardSet.username.charAt(0)}</button>
                        <p>{flashcardSet.username}</p>
                    </div>
                ...
                <div className={styles.setButtons}>
                    { user && (user.username === flashcardSet.username) && 
                        (<button onClick={handleEdit}
                            className={styles.editButton}><SquarePen className={styles.squarePen} />Edit
                        </button>)
                    }
                    <button className={styles.studyButton} onClick={handleStudy}>Study</button>
                </div>
            </div>
        </div>
    </>);
